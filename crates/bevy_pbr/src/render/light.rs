...
pub fn extract_lights(
    ...
    global_visible_lights: Extract<Res<GlobalVisibleClusterableObjects>>,
    cubemap_visible_entities: Extract<
        Query<(RenderEntity, &ViewVisibility), With<CubemapVisibleEntities>>,
    >,
    ...
    // `global_visible_lights` list anymore.
    commands.try_insert_batch(
        cubemap_visible_entities
            .iter()
            .map(|(render_entity, _)| (render_entity, RenderCubemapVisibleEntities::default()))
            .collect::<Vec<_>>(),
    );
    ...
    for entity in global_visible_lights.iter().copied() {
        let Ok((
            main_entity,
            render_entity,
            point_light,
